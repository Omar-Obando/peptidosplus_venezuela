...
Outside PhantomWP chat (Claude Code, Cursor, any MCP client), the same tools are served by the local PhantomWP MCP server at `.phantomwp/mcp/server.mjs`, registered in `.mcp.json` as `phantomwp` (discovery: `get_wordpress_schema`, `fetch_wp_sample`, `browse_content`; scaffold: `wp_register_post_type`, `wp_register_taxonomy`, `wp_register_field_group`, `wp_create_posts`, `wp_delete_managed_posts`, `wp_list_managed_scaffold` -- writes go through PhantomWP and need a PHANTOMWP_MCP_TOKEN plus scaffold writes enabled in wp-admin > PhantomWP; check `scaffold.writesEnabled` in `get_wordpress_schema`). Prefer those tools over hand-written WordPress REST exploration. If the server is not registered in your client, run: `claude mcp add phantomwp -- node .phantomwp/mcp/server.mjs`.
