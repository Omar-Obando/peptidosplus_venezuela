...
import { createGitConflictRuntime } from './git-conflict-runtime.js';
...
// Serializes history-rewriting git operations (push/rebase, force pull)
// against file writes. WebSocket messages are handled concurrently, so the
// IDE can rewrite managed files (e.g. src/styles/theme.css on a Theme Studio
// or preview sync) while a rebase is replaying commits -- git then dies with
// "Your local changes to the following files would be overwritten by merge".
// Writes wait for the git op to finish; git ops wait for each other.
let gitCriticalOp = null;

async function waitForGitQuiet() {
  while (gitCriticalOp) {
    try { await gitCriticalOp; } catch { /* holder reports its own errors */ }
  }
}

async function withGitLock(fn) {
  await waitForGitQuiet();
  const run = fn();
  gitCriticalOp = run.catch(() => { /* holder reports its own errors */ });
  try {
    return await run;
  } finally {
    gitCriticalOp = null;
...
async function abortRebaseIfActive() {
  for (const marker of ['rebase-merge', 'rebase-apply']) {
    ...
    try { await runGitCommand(['rebase', '--abort']); } catch { /* best-effort */ }
    return true;
  }
  return false;
}

// A rebase failure caused by files changing under it (as opposed to a real
// content conflict, which needs the user).
function isDirtyTreeRebaseError(message) {
  return /would be overwritten by merge|unstaged changes|uncommitted changes|cannot rebase: you have/i.test(String(message || ''));
}
...
const conflictRuntime = createGitConflictRuntime({
  workspaceDir: WORKSPACE_DIR,
  gitExec: runGitCommand,
  refreshGitCredentials,
});

const conflictBlockedActions = new Set([
  'write', 'write-batch', 'create', 'mkdir', 'delete', 'rename', 'exec',
  'git-commit', 'git-push', 'git-pull-force', 'git-branch-create',
  'git-branch-checkout', 'git-merge-into', 'git-delete-branch',
]);

function isConflictBlockedMutation(data) {
  if (conflictBlockedActions.has(data.action)) return true;
  if (data.action !== 'git') return false;
  const args = Array.isArray(data.args) ? data.args : [];
  const command = String(args[0] || '');
  if (command === 'tag') {
    return !(args.length === 1 || args[1] === '-l' || args[1] === '--list');
  }
  return !new Set(['status', 'diff', 'log', 'show']).has(command);
}
...
wss.on('connection', (ws, req) => {
  ...
  ws.on('message', async (message) => {
    try {
      ...
      if (isConflictBlockedMutation(data) && await conflictRuntime.isActive()) {
        sendWsResponse(ws, data, {
          action: data.action,
          success: false,
          code: 'conflict-session-active',
          error: 'Resolution is in progress. Finish or stop it before changing workspace files or Git history.',
        });
        return;
      }
      ...
      switch (data.action) {
        case 'git-conflict-start':
          sendWsResponse(ws, data, { action: data.action, ...await withGitLock(() => conflictRuntime.start()) });
          break;
        case 'git-conflict-status':
          sendWsResponse(ws, data, { action: data.action, ...await withGitLock(() => conflictRuntime.status(data)) });
          break;
        case 'git-conflict-read':
          sendWsResponse(ws, data, { action: data.action, ...await withGitLock(() => conflictRuntime.read(data)) });
          break;
        case 'git-conflict-save':
          sendWsResponse(ws, data, { action: data.action, ...await withGitLock(() => conflictRuntime.save(data)) });
          break;
        case 'git-conflict-finish':
          sendWsResponse(ws, data, { action: data.action, ...await withGitLock(() => conflictRuntime.finish(data)) });
          break;
        case 'git-conflict-abort':
          sendWsResponse(ws, data, { action: data.action, ...await withGitLock(() => conflictRuntime.abort(data)) });
          break;
        case 'git-conflict-recoveries':
          sendWsResponse(ws, data, { action: data.action, ...await withGitLock(() => conflictRuntime.recoveries()) });
          break;
        ...
        case 'write':
          try {
            // Hold writes while a push/rebase is replaying commits,
            // otherwise git aborts on the freshly-dirtied file.
            await waitForGitQuiet();
            const resolvedPath = assertSafeWorkspacePath(WORKSPACE_DIR, data.path);
        ...
        case 'git-push':
          try {
            await withGitLock(async () => {
              ...
              // A stale rebase left behind by an earlier failed push would
              // make every git step below fail -- clear it first. Nothing is
              // lost: aborting returns the branch to the user's last commit.
              await abortRebaseIfActive();
              ...
              try {
                ...
                if (behind > 0) {
                  for (let attempt = 0; ; attempt++) {
                    try {
                      ...
                      // Files changed while the rebase was replaying commits
                      // (writes from before the lock, or non-IDE writers like
                      // the dev server). Abort and retry: the next attempt's
                      // --autostash absorbs them.
                      if (attempt >= 2 || !isDirtyTreeRebaseError(rebaseError.message)) {
                        throw rebaseError;
                      }
                      await abortRebaseIfActive();
                    }
                ...
                if (rebaseTarget === '@{u}' || !String(error.message).includes('unknown revision')) {
                  // Don't leave a half-finished rebase behind unless it's a
                  // real content conflict the user may want to resolve.
                  if (isDirtyTreeRebaseError(error.message)) {
                    await abortRebaseIfActive();
                  }
                  throw error;
                }
