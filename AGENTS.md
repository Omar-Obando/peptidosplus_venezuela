...
- This project ships a local MCP server (`.phantomwp/mcp/server.mjs`, registered in `.mcp.json`) that exposes `get_wordpress_schema`, `fetch_wp_sample`, and `browse_content`, plus the `wp_*` scaffold tools (CPTs, taxonomies, field groups, seed content; gated by the wp-admin scaffold-writes toggle) to any MCP-capable agent. Prefer it over hand-written WordPress REST exploration when it is available.
